...
func TestModeline_NTSC240p(t *testing.T) {
	...
	fieldSizeFn := func() uint32 {
		return uint32(groovy.FieldPayloadBytes(
			groovy.NTSC240p60.HActive,
			groovy.NTSC240p60.VActive,
			groovy.NTSC240p60.Interlace,
			3,
		))
	}
	...
	plane := dataplane.NewPlane(dataplane.PlaneConfig{
		...
		SpawnSpec: ffmpeg.PipelineSpec{
			InputURL:        samplePath,
			OutputWidth:     int(groovy.NTSC240p60.HActive),
			OutputHeight:    int(groovy.NTSC240p60.VActive),
		...
		FieldWidth:    int(groovy.NTSC240p60.HActive),
		FieldHeight:   groovy.NTSC240p60.FieldHeight(),
		BytesPerPixel: 3,
	...
	for i, f := range snap.BlitFields {
		if f != 0 {
			t.Errorf("NTSC_240p: blit[%d] field bit = %d, want 0 (progressive)", i, f)
			break
		}
	}
	...
	if snap.AudioBytes < lo || snap.AudioBytes > hi {
		t.Errorf("NTSC_240p: audio bytes = %d, want %d±10%% [%d, %d]",
