...
func TestModeline_PAL288p(t *testing.T) {
	...
	fieldSizeFn := func() uint32 {
		return uint32(groovy.FieldPayloadBytes(
			groovy.PAL288p50.HActive,
			groovy.PAL288p50.VActive,
			groovy.PAL288p50.Interlace,
			3,
		))
	}
	...
	plane := dataplane.NewPlane(dataplane.PlaneConfig{
		...
		SpawnSpec: ffmpeg.PipelineSpec{
			InputURL:        samplePath,
			OutputWidth:     int(groovy.PAL288p50.HActive),
			OutputHeight:    int(groovy.PAL288p50.VActive),
			OutputFpsExpr:   "50/1",
		...
		FieldHeight:   groovy.PAL288p50.FieldHeight(),
		BytesPerPixel: 3,
	...
	for i, f := range snap.BlitFields {
		if f != 0 {
			t.Errorf("PAL_288p: blit[%d] field bit = %d, want 0 (progressive)", i, f)
			break
		}
	}
	...
	if snap.AudioBytes < lo || snap.AudioBytes > hi {
		t.Errorf("PAL_288p: audio bytes = %d, want %d±10%% [%d, %d]",
