...
gain to outgoing PCM via `scalePCMVolumeInPlace`
([plane.go:1436](../../../internal/dataplane/plane.go#L1436)), driven by an
`atomic.Int32` that `SetOutputVolume` updates live
([plane.go:509](../../../internal/dataplane/plane.go#L509)). We extend that
same stage with a biquad filter chain. The codebase already designs biquads
(`designShelfHigh`, `biquadCoeffs` in
[audiometer.go:456](../../../internal/dataplane/audiometer.go#L456)).
...
2. **Per-channel biquad state** (`z1/z2` history) — owned by the
   ...
   outside the swapped object, changing a knob mid-cast does not reset
   history → no clicks/pops.
...
int16 exactly once at the end: a 13-biquad int16 cascade would accumulate
rounding noise and could wrap.

## DSP specifics

Biquads use the RBJ Audio EQ Cookbook forms, designed at the session sample
rate (48000/44100/22050). Adds `designShelfLow`, `designPeaking`,
`designHighpass` next to the existing `designShelfHigh`.
...
| EQ 0–9 | peaking | 31·63·125·250·500·1k·2k·4k·8k·16k Hz | ~1.4 | each band dB |
...
- **Balance** is a per-channel linear gain; center = unity both channels.
- **Mono** folds L/R to their average before the cascade; on a mono source
  it is a no-op and balance is a no-op.
...
**Headroom/clipping:** stacked boosts can exceed 0 dBFS. v1 policy is a
**saturating clamp** at the int16 write (never wrap); the float pipeline
keeps intermediate stages clean. A future auto-makeup-trim is a non-goal.
...
- **A memory stores a voicing** (tone + 10 EQ bands), not the session
  toggles (mono/balance/loudness stay live across recalls). Three slots.
...
- **Template** `internal/chassis/templates/audio-strip.html`
  ...
  [shell.html](../../../internal/chassis/templates/shell.html) right after
  `transport`. The `.volume-control` block is **removed from**
...
- **`audiodsp` unit tests:** per-filter gain correctness (known sine at band
  ...
  balance law, and a **click-free swap** test (process across a coeff swap,
  assert no discontinuity at the boundary).
...
- **Clicks on knob changes** — mitigated by the fixed-slot chain + state
  living outside the atomically-swapped coeffs; covered by the click-free
  swap test.
- **Boost clipping** — saturating clamp prevents wraparound; float pipeline
  ...
  boost).
- **CPU per tick** — stereo × ~13 biquads in float at 48 kHz is modest, but
