...
| New chassis-owned struct `StreamsRefreshResult` | Return shape for the streams-refresh action — scalar status (source, duration, error), not per-provider slice |
...
3. **Streams section fully functional.** All 15 top-level fields render with autosave. The per-provider sub-section under a `.settings-subhead` heading shows one row per registered provider with the provider's `catalog_refresh_hours` override (the only per-provider field 4C deferred to 4D — `disabled` and `hls_buffer_disabled` stay owned by 4C's Catalog pane). A `↻ Refresh manifest now` action button at the bottom of the section invokes `StreamsRefresher.RefreshNow` (a single call to `streams.Adapter.RefreshNow(ctx, "")` — which is the canonical manifest-refresh entry point and what ripples to catalogs) and renders the scalar status (`source` / `duration` / `error`) in the row's `.action-result` slot. No per-provider fan-out; `streams.Adapter.RefreshNow(ctx, providerID)` only refreshes one provider's catalog (not the manifest) and is not the right primitive for this button.
...
| Section header structure | `<h4>{DisplayName} <span class="hint">{Kind} · {RuntimeState}</span></h4>`. Kind is a fixed per-adapter string (DLNA: `PUSH`, Torrent: `PASTE-IN · BT`, Streams: `PULL`); RuntimeState is computed at page render time from `IsEnabled()` + `Status().State` (DLNA: `LISTENING` / `DISABLED`; Torrent: omitted because there's no clean three-state runtime to surface without inventing one — header reads `Torrent · PASTE-IN · BT`; Streams: `{N} CHANNELS · see Catalog tab` where N is the channel sum from `StreamsCatalogViewer.Catalog()`, already wired in 4C). No polling — operator closes/reopens drawer to refresh. |
...
| Save route shape | `POST /receiver/settings/adapter/{name}` — one handler dispatching by adapter name. `{name}` matches the registered adapter ID (`dlna` / `torrent` / `streams`); unknown names return 404 + `{ok:false, chip:"UNKNOWN ADAPTER"}`. Body is `application/x-www-form-urlencoded`. Accepts any subset of the adapter's `Fields()` keys, including dotted per-provider keys (`providers.<id>.catalog_refresh_hours`). |
...
Section header: `<h4>DLNA <span class="hint">PUSH · {LISTENING|DISABLED}</span></h4>`. RuntimeState is `LISTENING` when `dlna.Adapter.Status().State == adapters.StateRunning`, else `DISABLED`. Using `Status().State` rather than `IsEnabled()` because the operator cares whether SSDP is actually advertising — `IsEnabled()` only reflects the config flag, which can be true while the adapter is still spinning up or has errored.
...
`tests/integration/` gains four new integration tests (per §Testing) that exercise the real `*uiserver.AdapterSaver` against a temp `config.toml`, validating disk-side round-trip and in-memory adapter state.
...
- `settings_adapter_dlna_save_test.go` — end-to-end POST against a real `*uiserver.AdapterSaver` with a temp `config.toml`, assert the disk-side TOML round-trips through `[adapters.dlna]`, the in-memory adapter state updates, and the wire scope matches the field's declared `ApplyScope`.
- `settings_adapter_torrent_save_test.go` — same shape, exercising RECAST scope on `download_dir` change against a temp `bridge.data_dir`.
- `settings_adapter_streams_save_test.go` — top-level field + per-provider key save in one POST; assert both land correctly in the disk-side TOML and the live `streams.Adapter.cfg`. Include an existing `[adapters.streams.providers.<id>.channels.<channel>]` subtable and assert it survives the partial save unchanged.
- `settings_action_streams_refresh_test.go` — real `streams.Adapter` with `refreshOnce` set to a fake that returns a deterministic `RefreshStatus` (success and failure variants). Assert the action route response matches the stub's `Source` / `Err` / measured duration, and that the chassis 30s context wraps the call (assert the fake observes a context with a deadline ≤ now+30s).
