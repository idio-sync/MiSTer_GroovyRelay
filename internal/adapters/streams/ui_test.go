...
func TestExtraPanelHTMLContainsStreamsPanel(t *testing.T) {
	...
	html = a.renderPanel(panelSelectionRequest{ProviderID: "toonami-aftermath", ProviderExplicit: true})
	for _, want := range []string{"Toonami Aftermath", "East", "West", "Movies", "Radio"} {
		if !strings.Contains(html, want) {
			t.Fatalf("panel missing %q: %s", want, html)
		}
	}
